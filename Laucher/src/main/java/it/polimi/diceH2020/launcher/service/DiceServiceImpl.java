...
/**
 * modelFile <-- model file to be modified to obtain input file inputFile <--
 * modified model file for the simulator
 * 
 * modelFile's path != inputFile's path inputFileDirectory the same of sol
 * 
...
public class DiceServiceImpl {
	...
	private final static String[] symExtensions = { ".def", ".net" };// symbolic
																		// extensions
																		// required
																		// by
																		// symbolic
	...
	public void initializeSimulationsV10(int size, double[] mapRate, double[] reduceRate, double[] thinkRate) {
		...
		createNetFile(modelFilesPathWithoutExtension, localFilePathWithoutExtension, String.valueOf(mapRate[0]), String.valueOf(reduceRate[0]), String.valueOf(thinkRate[0]));// creates
																																												// .net
																																												// local
																																												// file);//creates
																																												// .net
	...
	/**
	 * This class creates and sends DEF files, already created NET file[, STA
	 * file, DIS file] to remote simulator's simulations folder; executes the
	 * simulation, save STA solution file locally in the WS simulation folder.
	 * To track the completion of threads this method has to return a Future
	 * (used for join threads after the fork)
	 * 
	 * @param simulation
	 * @return
	 */
	...
	public Future<Float> startSimulationV10(Simulation simulation) {
		...
		try {
			...
			petrinet.sendFile(defLocalFilePathWithoutExtension + ".def", remoteFilePath + fileNameWithoutExtension + ".def"); // sends
																																// .def
																																// local
																																// file
																																// to
	 ...
	 * 
	 * @param netFileSourcePath
	 *            input path
	 * @param netFileDestinationPath
	 *            output path
	 */
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				if (line.contains("param2"))// map rate
					line = line.replace("param2", mapRate);// R1:
															// 7.6402345345e-05
															// R6 2.44000000e-04
				if (line.contains("param3"))// red rate
					line = line.replace("param3", reduceRate);// R1:
																// 1.8923434523e-04
																// R6:
																// 1.94000000e-04
	...
	/**
	 * Creates DEF file for V10 simulation. Each simulation has one DEF file (!=
	 * NET). This method receive in input 2 paths, the first is the path to the
	 * general PNmodel's DEF file that this class will modify, the other is the
	 * path where the PNmodel's modified file will be saved.
	 * 
	...
	public void initializeSimulationsV7(int size, double[] mapRate, double[] reduceRate, double[] thinkRate) {
		commonInitialization(size, mapRate, reduceRate, thinkRate);
		createNetFile2(modelFilesPathWithoutExtension, localFilePathWithoutExtension, String.valueOf(mapRate[0]), String.valueOf(mapRate[1]), String.valueOf(reduceRate[0]), String.valueOf(reduceRate[1]),
				String.valueOf(thinkRate[0]), String.valueOf(thinkRate[1]));
	}
	...
	private void commonInitialization(int size, double[] mapRate, double[] reduceRate, double[] thinkRate) {
		...
		modelFilesPathWithoutExtension = modelFilesPathWithoutExtension + modelInitialFileName; // to
																								// look
																								// for
																								// the
																								// files
		...
		f.mkdirs(); // mkdirs (with final s) creates all the relative
					// subdirectories specified in path
		...
		// int size =
		// ((int)Math.floor((maxNumCores-minCores)/step)+1)*(maxNumUsers-minUsers+1);
		if (size > 1) {
			stringForThreadDirectories = "{";
			// Creating array that contains path to all subdirectories of remote
			// simulation folder (1 subdirectory per thread)
	...
	public Future<Float> startSimulationV7(Simulation simulation) {
		...
		try {
			...
			createDefFile2(modelFilesPathWithoutExtension, defLocalFilePathWithoutExtension, String.valueOf(cores), String.valueOf(users), String.valueOf(reduce), String.valueOf(map), String.valueOf(users2),
					String.valueOf(cores2), String.valueOf(reduce2), String.valueOf(map2));
			petrinet.sendFile(defLocalFilePathWithoutExtension + ".def", remoteFilePath + fileNameWithoutExtension + ".def"); // sends
																																// .def
																																// local
																																// file
			...
			// petrinet.setStrinvalue("/usr/local/GreatSPN/scripts/swn_"+simulator_type+"_sim
			// /home/user/Desktop/"+remoteFilePathWithouExtension+" -a
			// "+accuracy+" -c 6");
			...
			x = petrinet.runSimulation2(simulator_type, remoteFilePath + fileNameWithoutExtension, accuracy,
					simulations_local_directory + "sim_users" + users + "_cores" + cores + "_usrs2" + users2 + "_cores2" + cores2 + ".sta", batchString);
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.contains("parmr1"))// map rate
					line = line.replace("parmr1", mapRate);// R1:
															// 7.6402345345e-05
															// R6 2.44000000e-04
				if (line.contains("parmr2"))// map rate
					line = line.replace("parmr2", mapRate2);// R1:
															// 7.6402345345e-05
															// R6 2.44000000e-04

				if (line.contains("parrr1"))// red rate
					line = line.replace("parrr1", reduceRate);// R1:
																// 1.8923434523e-04
																// R6:
																// 1.94000000e-04
				if (line.contains("parrr2"))// red rate
					line = line.replace("parrr2", reduceRate2);// R1:
																// 1.8923434523e-04
																// R6:
																// 1.94000000e-04
	...
	/**
	 * Creates DEF file for V7 simulation. Each simulation has one DEF file (!=
	 * NET). This method receive in input 2 paths, the first is the path to the
	 * general PNmodel's DEF file that this class will modify, the other is the
	 * path where the PNmodel's modified file will be saved.
	 * 
